...
$\log \text{read}$의 계수는 어떻게 해석해야 할까? read 값은 연속 값이므로 1%의 작은 값을 취하는 것이 가능하다. 따라서 근사 값으로 해석해도 무방하다. 앞서 보았 듯 read 값의 1% 증가는 $\beta * 0.01$ 만큼의 효과를 종속 변수에 준다. 즉, 약 0.17 만큼의 증가를 초래한다. 
